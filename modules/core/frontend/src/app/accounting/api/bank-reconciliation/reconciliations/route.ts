...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${BACKEND_URL}/api/v1/bank-reconciliation/reconciliations${queryString ? `?${queryString}` : ""}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${BACKEND_URL}/api/v1/bank-reconciliation/reconciliations`;
