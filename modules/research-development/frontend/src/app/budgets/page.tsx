...
export default function BudgetsPage() {
  ...
  const loadBudgets = async () => {
    try {
      const data = await api.rd_budgets.list();
      const budgetsArray = Array.isArray(data) ? data : (data.rd_budgets || data.data || data.items || []);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-12">
        <div>
          <h1 className="text-3xl font-bold text-black">R&D Budgets</h1>
        </div>
      ...
      <div className="card mb-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search budgets by name or fiscal year..."
            value={searchTerm}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-right py-3 px-4 font-semibold text-black">Allocated Amount</th>
                  <th className="text-right py-3 px-4 font-semibold text-black">Spent Amount</th>
                ...
                {filteredBudgets.map((budget) => (
                  <tr key={budget.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-right text-gray-600">
                      ${budget.allocated_amount ? budget.allocated_amount.toLocaleString() : '0'}
                    </td>
                    <td className="py-3 px-4 text-right text-gray-600">
                      ${budget.spent_amount ? budget.spent_amount.toLocaleString() : '0'}
                    </td>
